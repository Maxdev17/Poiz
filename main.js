...
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    const waMessage = `Hello Poiz Imaging Photo Studio 📸%0A%0AMy name is *${name}*%0APhone: ${phone}%0AService: ${service}%0A%0A${message ? message : 'I would like to book a session.'}`;
    window.open(`https://wa.me/2349134847773?text=${waMessage}`, '_blank');
  });
}

// --- FALLBACK: ensure all reveal elements become visible ---
// Safety net in case ScrollTrigger misses any element
window.addEventListener('load', () => {
  setTimeout(() => {
    document.querySelectorAll('.reveal, .reveal-left, .reveal-right, .reveal-scale').forEach(el => {
      if (getComputedStyle(el).opacity === '0') {
        gsap.to(el, { opacity: 1, y: 0, x: 0, scale: 1, duration: 0.6, ease: 'power2.out' });
      }
    });
  }, 2000);
});